...
var debug = require('debug')('loopback:explorer:modelHelper');
...
var modelHelper = module.exports = {
   ...
   */
  registerModelDefinition: function(modelCtor, typeRegistry, securityDefinitions) {
    ...
    typeRegistry.registerModel(name, function() {
      return definitionFunction(modelCtor, typeRegistry, securityDefinitions);
    });
  },
...
var definitionFunction = function(modelCtor, typeRegistry, securityDefinitions) {
  var lbdef = modelCtor.definition;
  ...
  addSwaggerSecurity(lbdef, typeRegistry, securityDefinitions);
  ...
  function addSwaggerSecurity(lbdef, typeRegistry, securityDefinitions) {
    var modelName = lbdef.name;
    var acls = lbdef.settings.acls;
    for (var i in acls) {
      var acl = acls[i];
      if (acl.principalType in securityDefinitions) {
        typeRegistry.routeSecurity(modelName, acl);
      }
    }
  };
};
