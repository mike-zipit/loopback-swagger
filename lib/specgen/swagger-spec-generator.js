...
module.exports = function createSwaggerObject(loopbackApplication, opts) {
  ...
  for (var modelName in models) {
    modelHelper.registerModelDefinition(models[modelName],
      typeRegistry, swaggerObject.securityDefinitions, opts);
  }
  ...
  for (var url in swaggerObject.paths) {
    for (var method in swaggerObject.paths[url]) {
      var security = typeRegistry.checkSecurity(
        swaggerObject.paths[url][method].operationId
      );
      if (security) {
