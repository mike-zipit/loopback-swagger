...
function TypeRegistry() {
  ...
  this._routeSecurities = Object.create(null);
...
TypeRegistry.prototype.routeSecurity = function(modelName, acl) {
  var key = modelName + '.' + acl.principalId;
  this._routeSecurities[key] = this._routeSecurities[key] || [];
  this._routeSecurities[key].push(acl);
};

TypeRegistry.prototype.checkSecurity = function(key) {
  var security = null;
  if (key in this._routeSecurities) {
    security = [];
    var obj = {};
    this._routeSecurities[key].forEach(function(acl) {
      obj[acl.principalType] = obj[acl.principalType] || [];
      obj[acl.principalType].push(acl.scope);
    });
    security.push(obj);
  }
  return security;
